...
/*COMPLETED BY MITUL KHENI*/
/*COMPLETED ON 4-28-2020*/
/*SUBMITTED TO MARK MORELL*/
...
GO
	BEGIN TRAN
	  ...
	  /*FIRST OF ALL THANK YOU FOR JOINING LATE LATE SHOW WITH ME */
	  
	  /*  
		FOR INSERT OPERATION WE HAVE TO SPECIFY COLUMN NAMES IN 'TABLE_NAME ( )' AND 
		VALUE OF THAT COLUMN CAN BE GIVEN IN 'VALUES ()' PART 
		REMEBER ONE THING THAT GIVEN VALUE MUST BE IN ORDER AS SAME AS COLUMN NAME, GIVEN IN INSERT OPERATION. 

		SO HOW TO INSERT A NEW EMPLOYEE IN TABLE?
		FIRST MENTION COLUMN NAMES AND THAN PROVIDE DATA IN VALUES ().
		...
		/*LET'S UPDATE VALUE OF M MARKEL'; WE NEED TO CHANGE POSITION FROM DBA TO DA. 
		  SO, FOR THAT YOU HAVE TO BE SURE THAT, YOU ARE CHANGING DATA OF M MARKEL NOT SOMEONE ELSE,
		  FOR THAT YOU CAN GIVE HIS ID OR NAME IN UPDATE'S WHERE CONDITION.
		  
		  UPDATE STRCUTURE :
		   UPDATE TABLENAME SET COL = VALUE, COL = VALUE.....
		   WHERE CONDITION. */

		/* WHAT IF THERE ARE 2 MARK IN EMPLOYEE TABLE?
	   	  THAN ALSO CONSIDER LASTNAME AS WELL. */
		...
		/*LET'S DELETE ONE OF OUR NEWLY INSERTED DATA FROM TABLE
			
			FOR DELETE YOU CAN DELETE SINGLE OR MULTIPLE RECORDS FROM TABLE
			SAME QUESTION? WHAT IS THERE ARE MORE THAN 1 DAVID IN OUR TABLE ?
			 IT WILL DELETE ANY RECORD WHICH HAS DAVID AS IT'S FIRST NAME - TO PREVENT THIS,
			 CONSIDER LAST NAME AS WELL. (SAME AS UPDATE)

			 DELETE STRUCTURE:
			 DELETE FROM TABLE_NAME 
			 WHERE CONDITION.

			 NOW LET'S DELETE DAVID ALLISON FROM OUR TABLE.
			 EXAMPLE */
		...
		/*NOW LET'S CHANGE SOME DATA FOR SPECIES TABLE AS WELL, REMEBER ALL THE BASIC CONCEPTS FROM ABOVE*/

		/* 
		 INSERT MULTIPLE SPECIES INTO SPECIES TABLE
		 REMEBER STRING NAME SHOULD BE IN ' '
		 DIGIT DOES NOT REQUIRED ' ' */
		INSERT INTO Animal.Species	(Name,Status,TotalExists)
		...
		/*LET'S DO SOME COMPLEX QUERY SUCH AS ONLY RETRIVE EMPLOYEE INFORMATION WHOS IMAGE STORED IN IMAGE TABLE*/
		SELECT *
		FROM Animal.Employee
...
PRINT'>>>************************************************';
PRINT'>>> FINALLY, THAT IS WRAP UP !!';
PRINT'>>> ENJOYED A LOT WITH YOU PROFESSOR, THANK YOU FOR TEACHING US.';
PRINT'>>>....... THANK YOU .......';
PRINT'>>> NOW IT IS TIME TO WATCH LORD OF THE RINGS : RETURN OF THE KING.';
PRINT'>>> LADIES AND GENTLEMEN ELVIS HAS LEFT THE BULDING !!';
PRINT'>>>************************************************';
